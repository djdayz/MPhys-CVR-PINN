...
def main():
    ...
    p.add_argument("--bold", type=Path, default=Path("/workspace/tCNR_0.500"), help="Single unseen BOLD file or directory of unseen BOLD files")
    p.add_argument("--outdir", type=Path, default=Path("/workspace/pinn_tcnr0.5_40"), help="Output root directory")
    p.add_argument("--pretrained-tissue", type=Path, default=Path("/workspace/pinn_tcnr0.5_60/best_tissue_net.pt"))
    p.add_argument("--pretrained-param", type=Path, default=Path("/workspace/pinn_tcnr0.5_60/best_param_net.pt"))
    ...
    p.add_argument("--delay-max", type=float, default=120.0)
    ...
    p.add_argument("--split-json", type=Path, default=Path("/workspace/pinn_tcnr0.5_60/data_split.json"))
    ...
    if "unused_bold_paths" not in split_info:
        raise ValueError(f"split_json file must contain 'unused_bold_paths' key with list of paths to adapt to.")
    ...
    if len(bold_paths) > 1:
        print(
            f"Multiple unseen BOLD files matched ({len(bold_paths)} total). "
            f"Only the first will be adapted: {bold_paths[0]}"
        )
    bold_paths = bold_paths[:1]
